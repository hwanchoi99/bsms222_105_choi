...
![Homicide per 100,000 in G-8 Countries](https://rafalab.github.io/dsbook/book_files/figure-html/murder-rate-example-1-1.png)
Or even worse, this version from everytown.org:  
![gun homicides per 100,000 residents](https://rafalab.github.io/dsbook/book_files/figure-html/murder-rate-example-2-1.png)
...
![Rstudio image](https://rafalab.github.io/dsbook/R/img/rstudio-environment.png)
We should see a, b, and c. if you try to recover the value of a variable that is not in your workspace, you receive an error. For example, if you type x, you will receive the following message:  
```{r}
x
```
...
?log
```
he help page will show you what arguments the function is expecting. For example, log needs x and base to run. However, some arguments are required and others are optional. You can determine which arguments are optional by noting in the help document that a default value is assigned with =. Defining these is optional. For example, the base of the function log defaults to base = exp(1) making log the natural log by default.
...
?"+"
```
and the relational operator by typing:
```{r}
help(">")
```
or
...
```
## 2.3 Exercises
1. What is the sum of the first 100 positive integers? The formula for the sum of integers  
1 through n is n(n+1)/2. Define n=100 and then use R to compute the sum of 1 through 100 using the formula.  
What is the sum?
```{r}
n<-100
n*(n+1)/2
...
```{r}
log(exp(x))  #answer is C, because log function have 'e' as default base value
```
## 2.4 Data types
...
names(murders)
```
t is important to know that the order of the entries in murders$population preserves the order of the rows in our data table. This will later permit us to manipulate one variable based on the results of another. For example, we will be able to order the state names by the number of murders.
...
record$student_id
```
We can also use double square brackets ( [[ ) likt this:
```{r}
record[["student_id"]]
```
...
names(codes)
```
if the use of strings without quotees looks confusing, know that you can use the quotes as well:
```{r}
codes <- c("italy" = 380, "canada" = 124, "egypt" = 818)
codes
```
...
seq(1,10)
```
he first argument defines the start, and the second defines the end which is included. The default is to go up in increments of 1, but a third argument lets us tell it how much to jump by:
```{r}
seq(1,10,2)
```
...
We said that v
ectors must be all of the same type. So if we try to combine, say, numbers and characters, you might expect an error:
```{r}
x <- c(1, "cananda", 3)
```
...
```{r}
x <- 1:5 #class(x) is 'integer' right now.
y <- as.character(x)
y
```
...
x <- c("1", "3", "5")
x <- as.integer(x)
class(x) #confirm that class of x is 'integer'
```
...
sum(ind)
table(ind) # I can use this function too.
# The answer is 145.
```
...
# Here murders$state%in%c("New York", "Florida", "Texas"), note that murders$state is first vector here. not second vector.
```
...
#I made my own histogram for tests.
state_population <- log10(murders$population)
hist(state_population, xlab="log10(popluation)", main="State Populations for test",sub="population")
```
